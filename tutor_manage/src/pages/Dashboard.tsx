      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        >
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-lg overflow-hidden flex flex-col max-h-[600px]">
            ...
            <div className="p-6 border-b border-white/20 bg-white/10 flex justify-between items-center z-10 relative gap-3">
              <div>
                <h3 className="font-extrabold text-white tracking-tight text-lg">Pending Fees Tracker</h3>
                <span className="bg-white/20 border border-white/30 text-white text-xs font-black px-3 py-1 rounded-full shadow-inner mt-1 inline-block">{unpaidStudentsList.length} Due</span>
              </div>
              {unpaidStudentsList.length > 1 && (
                <button
                  onClick={() => {
                    unpaidStudentsList.forEach(item => {
                      if (item.student) {
                        const message = `Hi ${item.student.name},\n\nThis is a gentle reminder that your tuition fee of ₹${item.fee.amount} for this month is pending. Please process the payment at your earliest convenience.\n\nThank you!`;
                        window.open(`https://wa.me/${item.student.mobileNumber.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`, '_blank');
                      }
                    });
                    toast.success(`Opened WhatsApp for ${unpaidStudentsList.length} students`);
                  }}
                  className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 bg-[#25D366]/20 hover:bg-[#25D366]/30 text-white text-xs font-bold rounded-xl border border-[#25D366]/40 transition-all hover:scale-[1.02]"
                  title="Send reminder to all pending students"
                >
