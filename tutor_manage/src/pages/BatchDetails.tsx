            ...
            {activeTab === 'fees' && (
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/10 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  ...
                  {feeRecords.filter(f => f.status === 'Unpaid').length > 0 && (
                    <button
                      onClick={() => {
                        const unpaid = feeRecords.filter(f => f.status === 'Unpaid');
                        unpaid.forEach(fee => {
                          ...
                          if (student) {
                            ...
                            window.open(`https://wa.me/${student.mobileNumber.replace(/\D/g, '')}?text=${encodeURIComponent(msg)}`, '_blank');
                          }
                        });
                        toast.success(`Opened WhatsApp for ${unpaid.length} students`);
                      }}
                      className="flex items-center gap-2 bg-[#25D366]/20 hover:bg-[#25D366]/30 border border-[#25D366]/50 text-white px-5 py-3 rounded-2xl text-sm font-bold transition-all hover:scale-[1.02] w-full sm:w-auto whitespace-nowrap"
                    >
                      ...
                      Remind All ({feeRecords.filter(f => f.status === 'Unpaid').length})
                    </button>
                  )}
                  <button
